#.data #
#	vector: .word 0xf 0xf 0xf 0xf  
#	row_0: .word 1 2 3 4  ##offset 
#	row_1: .word 5 6 7 8 
...
main:
	...
	#la s3, row_2
	#la s4, row_3
	auipc s0, 0x0000fc10 ## vector
	addi s0, s0, 0
	auipc s1, 0x0000fc10 ## 
	
	## -------------------------	
	## ASSIGNED IN CLEAN_REG!!!
	## -------------------------
	#addi s1, s1, 8
	#add s1, s0, zero
	auipc s3, 0x0000fc10 # result start 
	addi s3, s3, 0x40 # result start
	...
	addi t3, s0, 0x50
	#li t2, 0x8 # value fetched from uart
	#addi t4, zero, zero # restore t4 
	addi t6, zero, 1 
...
population_loop:
	...
	sw t2, 0(s1)  # store t2 in mem 
	addi s1, s1, 4 ## vector + 4 
	beq s1, t3, clean_reg # finished populating vector and 4 rows? 
